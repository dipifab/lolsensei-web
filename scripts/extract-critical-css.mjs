...
const LOCALES = ['en', 'zh-hans']; // worst-case glyph density (latin + CJK)
const VIEWPORT = { width: 375, height: 812, deviceScaleFactor: 2, isMobile: true };
const BUDGET_CONTENT_BYTES = 14 * 1024; // 14336, target contenuto critical (REQ-NF-019-1-002)
...
async function run() {
  ...
  try {
    ...
    for (const locale of LOCALES) {
      ...
      // Settle: lascia girare eventuali effect Solid ATF
  ...
  // Minify basic: collapse whitespace
  ...
  if (minified.length > BUDGET_CONTENT_BYTES) {
    ...
    log(`  Still writing file; caller gate (TASK-3-002) will fail on build if combined with fonts.css > 14336 bytes.`);
  }
