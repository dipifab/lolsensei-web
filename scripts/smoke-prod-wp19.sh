...
# S1 already covered by HTTP 200 check above. Explicit labeled below for traceability.
...
# S3: <style> inner content size > 1024 byte (deterministic Node, BSD-safe, TASK-3-002 aligned)
...
# S4: @font-face count >= 2 (Inter + Manrope minimum)
...
# S5: hero-panel-mobile.webp referenced in HTML
...
# S6: WOFF2 inter-latin accessibili con Content-Type corretto
...
# S9: No onload= on <link> tags (CSP strict compliance REQ-NF-019-1-006)
...
if [ "$onload_count" -eq 0 ]; then
  ...
  echo "  [FAIL] S9 onload= on <link> detected ($onload_count) — CSP async-CSS pattern forbidden"
  FAIL=$((FAIL + 1))
fi
