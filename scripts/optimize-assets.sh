...
BUDGET_LATIN=12288
BUDGET_LATIN_EXT=15360
BUDGET_TOTAL=25600
BUDGET_HERO=81920

# WP19.1 tightened ranges (rimossi PUA/general-punctuation/symbol per budget strict)
...
if [[ ! -f "${SRC_TTF}" ]]; then
  ...
  if [[ -z "${FOUND_TTF}" ]]; then
    err "fetch: Inter-Regular.ttf not found in zip"
    exit 3
  fi
...
if ! echo "${TTX_HEAD}" | grep -q 'Inter'; then
  err "preflight: TTF family does not contain 'Inter'"
  exit 1
fi
...
if (( SIZE_HERO > BUDGET_HERO )); then
  err "hero-panel-mobile.webp: ${SIZE_HERO} bytes > ${BUDGET_HERO} budget"
  exit 6
fi
...
log "Remember: public/fonts/fonts.css unicode-range must mirror UNICODES_LATIN/UNICODES_LATIN_EXT."
