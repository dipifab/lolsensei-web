...
// Output dei full body delle guide, raggruppati per (lang, slug). Caricati
// in KV `CHAMPION_GUIDES` via `npm run kv:upload` e fallback locale durante
// `vinxi dev`. Fuori da `src/` per evitare che vengano bundlati nel Worker.
const KV_OUTPUT_DIR = resolve(REPO_ROOT, 'dist/kv-uploads/champions');
const LANGS = SUPPORTED_LOCALES;
const ROLES = ['top', 'jungle', 'mid', 'bot', 'support'];

/** Converte uno slug locale (es. 'en', 'pt-br', 'zh-hans') nella forma
 *  camelCase richiesta dagli identifier TypeScript:
 *    'en'      -> 'en'
 *    'pt-br'   -> 'ptBr'
 *    'zh-hans' -> 'zhHans'
 */
function localeToCamel(lang) {
  return lang.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
}
...
//
// Per ogni locale supportato esiste un marker H2 canonical (deve combaciare
// con il set CANONICAL_H2 in scripts/content-lint.mjs). Quando matchup_draft
// e' presente, il compile splitta il body sull'header H2 di "Key matchups"
// localizzato e renderizza il componente <MatchupDraft /> tra Recommended
// Build e Key matchups.
//
// WP35.1 — esteso da EN+IT a tutte le 8 lingue del sito.
const MATCHUP_SPLIT_MARKERS = {
  en: /^## Key matchups\s*$/m,
  it: /^## Matchup chiave\s*$/m,
  es: /^## Matchups clave\s*$/m,
  fr: /^## Matchups clés\s*$/m,
  de: /^## Wichtige Matchups\s*$/m,
  'pt-br': /^## Matchups principais\s*$/m,
  ko: /^## 주요 매치업\s*$/m,
  'zh-hans': /^## 关键对线\s*$/m,
};
...
function emitTsModule(lang, guides) {
  // Identifier TS-safe: 'pt-br' -> 'ptBrChampions', 'zh-hans' -> 'zhHansChampions'.
  const constName = `${localeToCamel(lang)}Champions`;
  return [
...
async function main() {
  ...
  await mkdir(KV_OUTPUT_DIR, { recursive: true });
  ...
  for (const lang of LANGS) {
    ...
    // Emetti un file JSON per (lang, slug) — destinazione KV `guides:<lang>:<slug>`.
    // Una guida singola di solito (raramente piu' patch storiche per lo stesso slug).
    const langDir = resolve(KV_OUTPUT_DIR, lang);
    await mkdir(langDir, { recursive: true });
    const bySlug = new Map();
    for (const g of guides) {
      const slug = `${g.champion}-${g.role}`;
      if (!bySlug.has(slug)) bySlug.set(slug, []);
      bySlug.get(slug).push(g);
    }
    for (const [slug, list] of bySlug) {
      await writeFile(
        resolve(langDir, `${slug}.json`),
        JSON.stringify(list),
        'utf8',
      );
      totalKvFiles += 1;
    }
    console.log(`[compile-content] ${lang}: ${guides.length} guide(s) emitted across ${bySlug.size} KV file(s)`);
  ...
  console.log(
    `[compile-content] done: ${totalGuides} total guide(s), ${totalKvFiles} KV file(s) ready in dist/kv-uploads/champions/.`,
  );
}
