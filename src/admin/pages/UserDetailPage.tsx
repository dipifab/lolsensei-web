...
import { AdminUsersApi, type SetLimitBody } from '../api/users';
...
export default function UserDetailPage() {
  ...
  const [profile, { refetch: refetchProfile }] = createResource(
    ...
    async (id) => {
      try {
        return await AdminUsersApi.detail(id);
      } catch (err) {
  ...
  const [timeline, { refetch: refetchTimeline }] = createResource(
    ...
    async (id) => {
      try {
        return await AdminUsersApi.timeline(id, { page: 1, page_size: 50 });
      } catch (err) {
        if (err instanceof AdminApiError && err.status !== 401 && err.status !== 404) {
          toast.showToast(err.message, 'error');
        }
        throw err;
      }
  ...
  const [billing, { refetch: refetchBilling }] = createResource(
    ...
    async (src) => {
      if (!src) return null;
      try {
        return await AdminUsersApi.billing(src.id, { page: src.page, page_size: 50 });
      } catch (err) {
  ...
  const [aiUsage] = createResource(aiSource, async (src) => {
    if (!src) return null;
    try {
      return await AdminUsersApi.aiUsage(src.id, { from: src.from, to: src.to });
    } catch (err) {
      if (err instanceof AdminApiError && err.status !== 401) {
        toast.showToast(err.message, 'error');
      }
  ...
  const doSuspend = async (reason?: string) => {
    ...
    try {
      ...
      if (err instanceof AdminApiError) toast.showToast(err.message, 'error');
    } finally {
      setMutating(false);
    }
  };
  ...
  const doUnsuspend = async (reason?: string) => {
    ...
    setMutating(true);
    try {
      await AdminUsersApi.unsuspend(userId(), reason);
  ...
  const doSetLimit = async (body: SetLimitBody) => {
    ...
    try {
      await AdminUsersApi.setLimit(userId(), body);
  ...
  const doRemoveLimit = async (reason: string) => {
    ...
    try {
      await AdminUsersApi.removeLimit(userId(), reason);
        ...
        <Show
          ...
          fallback={
            <div class="bg-surface-container rounded-xl p-6">
              ...
              <div class="mt-3">
                <AdminSkeleton variant="card" height="4rem" />
              </div>
            </div>
          }
        >
                ...
                <div class="flex items-center justify-between gap-4 flex-wrap">
                  ...
                  <div class="flex items-center gap-2 flex-wrap">
                    <Show when={p().plan_name}>
                      <AdminBadge variant="trialing" label={p().plan_name!} />
                    </Show>
                    <AdminBadge
              ...
              >
                ...
                <Show when={tab() === 'billing'}>
                  <div class="space-y-4">
                    <Show
                      ...
                      fallback={
                        <div class="space-y-3">
                          <AdminSkeleton variant="card" height="10rem" />
                          <AdminSkeleton variant="card" height="16rem" />
                        </div>
                      }
                    >
                      {(b) => (
                ...
                <Show when={tab() === 'ai-usage'}>
                  <div class="space-y-4">
                    ...
                    <Show
                      when={aiUsage()}
                      fallback={<AdminSkeleton variant="card" height="12rem" />}
                    >
                      {(a) => (
                        <>
