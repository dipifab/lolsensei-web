...
export const AdminUsersApi = {
  list: (p: UserListParams = {}) =>
    adminFetch<UserListResponse>(`/api/admin/users${qs(p as Record<string, unknown>)}`),

  detail: (id: string) =>
    adminFetch<UserDetailResponse>(`/api/admin/users/${id}`),

  suspend: (id: string, reason: string) =>
    adminFetch<SuspendResponse>(`/api/admin/users/${id}/suspend`, {
      method: 'PUT',
      body: JSON.stringify({ reason }),
    }),

  unsuspend: (id: string, reason: string) =>
    adminFetch<SuspendResponse>(`/api/admin/users/${id}/unsuspend`, {
      method: 'PUT',
      body: JSON.stringify({ reason }),
    }),

  setLimit: (id: string, body: SetLimitBody) =>
    adminFetch<LimitResponse>(`/api/admin/users/${id}/limit`, {
      method: 'PUT',
