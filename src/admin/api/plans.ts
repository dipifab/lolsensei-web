...
export function fetchPlans(isActive?: boolean): Promise<PlanResponse[]> {
  ...
  return adminFetch<PlanResponse[]>(`/api/admin/plans${params}`);
}

export function createPlan(data: PlanCreateRequest): Promise<PlanResponse> {
  return adminFetch<PlanResponse>('/api/admin/plans', {
    method: 'POST',
...
export function updatePlan(
  ...
  return adminFetch<PlanResponse>(`/api/admin/plans/${planId}`, {
    method: 'PUT',
    body: JSON.stringify(data),
  });
}
